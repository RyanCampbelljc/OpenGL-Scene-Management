...
Program::Program(const std::string& vsFile, const std::string& fsFile)
{
    ...
    if (pFileContents)
        ...
        std::cerr << "Failed to load file " << vsFile << std::endl;
    ...
    if (pFileContents)
        ...
        std::cerr << "Failed to load file " << fsFile << std::endl;
}
...
{
    GLint iStatus;
    const GLchar* sources[] = {preamble.c_str(), src.c_str()};
        ...
        switch (it->second.type)
        {
            ...
            case UniformType::Matrix4Array:
                if (it->second.m4Array.size() > 0)
                    glUniformMatrix4fv(
                        locations[it->first], (GLint)it->second.m4Array.size(), GL_FALSE,
                        glm::value_ptr(it->second.m4Array[0]));
                break;
