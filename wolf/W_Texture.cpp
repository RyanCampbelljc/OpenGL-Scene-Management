...
Texture::Texture(const std::string& path)
	: m_minFilter(Texture::FM_Invalid)
	, m_magFilter(Texture::FM_Invalid)
	, m_uWrap(Texture::WM_Invalid)
	, m_vWrap(Texture::WM_Invalid)
	, m_width(0)
	, m_height(0)
	, m_glTex(0)
{
	if (path.find(".dds") != std::string::npos)
		...
		path.find(".tga") != std::string::npos || path.find(".png") != std::string::npos
	...
	, m_glTex(0)
{
	...
	glTexImage2D(
		GL_TEXTURE_2D, 0, gs_aInternalFormatMap[fmt], width, height, 0, gs_aFormatMap[fmt], gs_aTypeMap[fmt], pData);
	...
	, m_glTex(0)
{
	...
	glTexImage2D(
		GL_TEXTURE_2D, 0, gs_aInternalFormatMap[fmt], width, height, 0, gs_aFormatMap[fmt], gs_aTypeMap[fmt], nullptr);
